PROGRAM main
  ...
  dat2 = dat1 + sin(dat1)
  isgood = .true.
  if (abs(variance(dat2,ddof=1) - autocoeffk(dat2,0,ddof=1)) > epsilon(1.0_dp)) isgood =.false.
  if (abs(autocorr(dat2,0) - 1.0_dp) > epsilon(1.0_dp)) isgood =.false.
  if (abs(covariance(dat1,dat2,ddof=1) - crosscoeffk(dat1,dat2,0,ddof=1))> epsilon(1.0_dp)) isgood =.false.
  ...
  if (abs(dc(1)/real(nout-1,dp) - covariance(dat1,dat2,ddof=1)) > 0.01_dp) isgood =.false.
  ...
  sat2 = sat1 + sin(sat1)
  isgood = .true.
  if (nint(variance(sat2,ddof=1)) /= nint(autocoeffk(sat2,0,ddof=1))) isgood =.false.
  if (abs(autocorr(sat2,0) - 1.0_sp) > epsilon(1.0_sp)) isgood =.false.
  if (abs(covariance(sat1,sat2,ddof=1) - crosscoeffk(sat1,sat2,0,ddof=1))> epsilon(1.0_sp)) isgood =.false.
  ...
  if (abs(sc(1)/real(nout-1,sp) - covariance(sat1,sat2,ddof=1)) > 0.01_sp) isgood =.false.
