...
CONTAINS
  ...
  function nelminrange(func, pstart, prange, funcmin, varmin, step, konvge, maxeval, &
    ...
    !  Check the input parameters.
    !
    if (size(prange,2) .ne. 2_i4)           stop 'nelminrange: range has to be array of size (:,2)'
    if (size(prange,1) .ne.  size(ipstart)) stop 'nelminrange: range has to be given for each dimension'
    if (.not. (all(prange(:,1) .le. pstart(:)) .and. all(prange(:,2) .ge. pstart(:)) )) then
       stop 'nelminrange: starting point is not in range'
    end if
    !
    !  Initialization.
    !
    ...
    do
       ...
       !
       ! bound to range
       nelminrange(1:n) = min( prange(1:n,2) , max( prange(1:n,1) , p(1:n,ilo) ) ) 
       ifuncmin   = y(ilo)
       ...
       !
       ! bound to range: probably not necessary
       ipstart(1:n) = min( prange(1:n,2) , max( prange(1:n,1) , nelminrange(1:n) ) ) 
       del          = eps
       inumrestart  = inumrestart + 1
