...
!>          those points, and which can be evaluated everywhere. Thus, it is possible to get a formula
!>          that allows you to "connect the dots".
!>
!>          Of course, you could could just connect the dots with straight lines, but that would look ugly,
!>          and if there really is some function that explains your data, you'd expect it to curve around
!>          rather than make sudden angular turns. The functions in SPLINE offer a variety of choices for
...
!>
!>          Also included are a set of routines that return the local "basis matrix", which allows the
!>          evaluation of the spline in terms of local function data.

!> \authors John Burkardt, Matthias Cuntz (CHS)
!> \date Jul 2013
module mo_spline

  ! This module is part of the UFZ CHS Fortran library.

  ! Adapted to the CHS library by Matthias Cuntz, July 2013
...
contains
  ...
  function spline_b(tval, tdata, ydata)
    ...
    !  Parameters:
    !
    !    Input, real(dp) TVAL, the points at which the spline is to be evaluated.
    !
    !    Input, real(dp) TDATA(NDATA), the abscissas of the data.
    !
    !    Input, real(dp) YDATA(NDATA), the data values.
    !
    !    Output, real(dp) SPLINE_B, the values of the function at TVAL.
    !
    implicit none
    ...
    real(dp), dimension(size(tval))    :: spline_b
    ...
    elsewhere
       spline_b = spline_b + ( 2.0_dp * ydata(1) - ydata(2) ) * bval
    end where
    !
    ...
    elsewhere
       spline_b = spline_b + ( 2.0_dp * ydata(ndata) - ydata(ndata-1) ) * bval
    end where
  ...
  subroutine basis_function_b_val ( tdata, tval, yval )
    ...
    !  Parameters:
    !
    !    Input, real(dp) TDATA(5), the nodes associated with the
    !    basis function.  The entries of TDATA are assumed to be distinct
    !    and increasing.
    !
    !    Input, real(dp) TVAL, a point at which the B spline basis
    !    function is to be evaluated.
    !
    !    Output, real(dp) YVAL, the value of the function at TVAL.
    !
    implicit none
  ...
  subroutine basis_function_beta_val ( beta1, beta2, tdata, tval, yval )
    ...
    !  Parameters:
    !
    !    Input, real(dp) BETA1, the skew or bias parameter.
    !    BETA1 = 1 for no skew or bias.
    !
    !    Input, real(dp) BETA2, the tension parameter.
    ...
    !    and increasing.
    !
    !    Input, real(dp) TVAL, a point at which the B spline
  ...
  subroutine basis_matrix_b_uni ( mbasis )
    ...
    !  Parameters:
    !
    !    Output, real(dp) MBASIS(4,4), the basis matrix.
    !
    implicit none
  ...
  subroutine basis_matrix_overhauser_nul ( alpha, mbasis )
    ...
    !    ALPHA = || P2 - P1 || / ( || P3 - P2 || + || P2 - P1 || )
    !
    !    Output, real(dp) MBASIS(3,3), the basis matrix.
    !
    implicit none
  ...
  subroutine basis_matrix_tmp ( left, n, mbasis, ndata, tdata, ydata, tval, yval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) LEFT, indicats that TVAL is in the interval
    !    [ TDATA(LEFT), TDATA(LEFT+1) ], or that this is the "nearest"
    !    interval to TVAL.
    !    For TVAL < TDATA(1), use LEFT = 1.
    !    For TDATA(NDATA) < TVAL, use LEFT = NDATA - 1.
    !
    !    Input, integer(i4) N, the order of the basis matrix.
    !
    !    Input, real(dp) MBASIS(N,N), the basis matrix.
    !
    !    Input, integer(i4) NDATA, the dimension of the vectors TDATA
    !    and YDATA.
    !
    !    Input, real(dp) TDATA(NDATA), the abscissa values.  This routine
    !    assumes that the TDATA values are uniformly spaced, with an
    !    increment of 1.0.
    !
    !    Input, real(dp) YDATA(NDATA), the data values to be
    !    interpolated or approximated.
    !
    !    Input, real(dp) TVAL, the value of T at which the spline is to be
    !    evaluated.
    !
    !    Output, real(dp) YVAL, the value of the spline at TVAL.
    !
    implicit none

    integer(i4), parameter :: maxn = 4
    integer(i4) n
    integer(i4) ndata
  ...
  subroutine bc_val ( n, t, xcon, ycon, xval, yval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the degree of the Bezier curve.
    !    N must be at least 0.
    !
    !    Input, real(dp) T, the point at which the Bezier curve should
    !    be evaluated.  The best results are obtained within the interval
    !    [0,1] but T may be anywhere.
    !
    !    Input, real(dp) XCON(0:N), YCON(0:N), the X and Y coordinates
    ...
    !
    !    Output, real(dp) XVAL, YVAL, the X and Y coordinates of the point
  ...
  function bez_val ( n, x, a, b, y )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the degree of the Bezier function.
    !    N must be at least 0.
    !
    !    Input, real(dp) X, the point at which the Bezier function should
    !    be evaluated.  The best results are obtained within the interval
    !    [A,B] but X may be anywhere.
    !
    !    Input, real(dp) A, B, the interval over which the Bezier function
    ...
    !
    !    Input, real(dp) Y(0:N), a set of data defining the Y coordinates
    !    of the control points.
    !
    !    Output, real(dp) BEZ_VAL, the value of the Bezier function at X.
    !
    implicit none
  ...
  subroutine bp01 ( n, x, bern )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the degree of the Bernstein basis 
    !    polynomials.  N must be at least 0.
    !
    !    Input, real(dp) X, the evaluation point.
    !
    !    Output, real(dp) BERN(0:N), the values of the N+1 Bernstein basis
    !    polynomials at X.
    !
    implicit none
  ...
  subroutine bpab ( n, a, b, x, bern )
    ...
    !    N must be at least 0.
    !
    !    Input, real(dp) A, B, the endpoints of the interval on which the
    !    polynomials are to be based.  A and B should not be equal.
    !    Input, real(dp) X, the point at which the polynomials are to be
  ...
  subroutine chfev ( x1, x2, f1, f2, d1, d2, ne, xe, fe, next, ierr )
    ...
    !    X2, respectively.
    !
    !    Input, real(dp) D1, D2, the derivative values at X1 and
    !    X2, respectively.
    !
    !    Input, integer(i4) NE, the number of evaluation points.
    !
    !    Input, real(dp) XE(NE), the points at which the function is to
    !    be evaluated.  If any of the XE are outside the interval
    !    [X1,X2], a warning error is returned in NEXT.
    !
    !    Output, real(dp) FE(NE), the value of the cubic function
    !    at the points XE.
    !
    !    Output, integer(i4) NEXT(2), indicates the number of
  ...
  subroutine data_to_dif ( ntab, xtab, ytab, diftab )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) NTAB, the number of pairs of points
    !    (XTAB(I),YTAB(I)) which are to be used as data.  The
    !    number of entries to be used in DIFTAB, XTAB and YTAB.
    !
    !    Input, real(dp) XTAB(NTAB), the X values at which data was taken.
    !    These values must be distinct.
    !
    !    Input, real(dp) YTAB(NTAB), the corresponding Y values.
    !
    !    Output, real(dp) DIFTAB(NTAB), the divided difference coefficients
  ...
  subroutine dif_val ( ntab, xtab, diftab, xval, yval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) NTAB, the number of divided difference
    !    coefficients in DIFTAB, and the number of points XTAB.
    !
    !    Input, real(dp) XTAB(NTAB), the X values upon which the
    !    divided difference polynomial is based.
    !
    !    Input, real(dp) DIFTAB(NTAB), the divided difference
    !    polynomial coefficients.
    !
    !    Input, real(dp) XVAL, the value where the polynomial
    !    is to be evaluated.
    !
    !    Output, real(dp) YVAL, the value of the polynomial at XVAL.
    !
    implicit none
  ...
  subroutine least_set_old ( ntab, xtab, ytab, ndeg, ptab, b, c, d, eps, ierror )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) NTAB, the number of data points.
    !
    !    Input, real(dp) XTAB(NTAB), the X data.  The values in XTAB
    !    should be distinct, and in increasing order.
    !
    !    Input, real(dp) YTAB(NTAB), the Y data values corresponding
    !    to the X data in XTAB.
    !
    !    Input, integer(i4) NDEG, the degree of the polynomial which the
    !    program is to use.  NDEG must be at least 0, and less than or
    !    equal to NTAB-1.
    !
    !    Output, real(dp) PTAB(NTAB), the value of the least
    !    squares polynomial at the points XTAB(1:NTAB).
    !
    !    Output, real(dp) B(1:NDEG), C(0:NDEG), D(2:NDEG), arrays
    !    needed to evaluate the polynomial.
    !
    !    Output, real(dp) EPS, the root-mean-square discrepancy of the
    !    polynomial fit.
    !
    !    Output, integer(i4) IERROR, error flag.
    ...
    rn0 = ntab
    c(0) = y_sum / real ( ntab, kind=dp )

    ptab(1:ntab) = y_sum / real ( ntab, kind=dp )

    if ( ndeg == 0 ) then
       eps = sum ( ( ptab(1:ntab) - ytab(1:ntab) )**2 )
       eps = sqrt ( eps / real ( ntab, kind=dp ) )
       return
    end if
    !
    !  The polynomial is of degree at least 1.
    !
    b(1) = sum ( xtab(1:ntab) ) / real ( ntab, kind=dp )
  ...
  subroutine least_val_old ( x, ndeg, b, c, d, value )
    ...
    !    to be evaluated.
    !
    !    Input, integer(i4) NDEG, the degree of the least squares
    !    polynomial.
    !
    !    Input, real(dp) B(1:NDEG), C(0:NDEG), D(2:NDEG), arrays
    !    defined by LEAST_SET_OLD, and needed to evaluate the polynomial.
    !
    !    Output, real(dp) VALUE, the value of the polynomial at X.
    !
    implicit none
  ...
  subroutine least_set ( point_num, x, f, w, nterms, b, c, d )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) POINT_NUM, the number of data values.
    !
    !    Input, real(dp) X(POINT_NUM), the abscissas of the data points.
    !    At least NTERMS of the values in X must be distinct.
    !
    !    Input, real(dp) F(POINT_NUM), the data values at the points X(*).
    !
    !    Input, real(dp) W(POINT_NUM), the weights associated with
    !    the data points.  Each entry of W should be positive.
    !
    !    Input, integer(i4) NTERMS, the number of terms to use in the
    !    approximating polynomial.  NTERMS must be at least 1.
    !    The degree of the polynomial is NTERMS-1.
    !
    !    Output, real(dp) B(NTERMS), C(NTERMS), D(NTERMS), are quantities
  ...
  subroutine least_val ( nterms, b, c, d, x, px )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) NTERMS, the number of terms in the least
    ...
    !
    !    Input, real(dp) B(NTERMS), C(NTERMS), D(NTERMS), the information
    !    computed by LEAST_SET.
    !
    !    Input, real(dp) X, the point at which the least squares polynomial
    !    is to be evaluated.
    !
    !    Output, real(dp) PX, the value of the least squares
    !    polynomial at X.
    !
    implicit none
  ...
  subroutine least_val2 ( nterms, b, c, d, x, px, pxp )
    ...
    !    is to be evaluated.
    !
    !    Output, real(dp) PX, PXP, the value and derivative of the least
  ...
  subroutine parabola_val2 ( dim_num, ndata, tdata, ydata, left, tval, yval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) DIM_NUM, the dimension of a single data point.
    !    DIM_NUM must be at least 1.
    !
    !    Input, integer(i4) NDATA, the number of data points.
    !    NDATA must be at least 3.
    !
    !    Input, real(dp) TDATA(NDATA), the abscissas of the data
    !    points.  The values in TDATA must be in strictly ascending order.
    !
    !    Input, real(dp) YDATA(DIM_NUM,NDATA), the data points
    !    corresponding to the abscissas.
    !
    !    Input, integer(i4) LEFT, the location of the first of the three
    !    consecutive data points through which the parabolic interpolant
    !    must pass.  1 <= LEFT <= NDATA - 2.
    !
    !    Input, real(dp) TVAL, the value of T at which the parabolic
    ...
    !
    !    Output, real(dp) YVAL(DIM_NUM), the value of the parabolic
    !    interpolant at TVAL.
    !
    implicit none
  ...
  function r8_uniform_01 ( seed )
    ...
    !  Parameters:
    !
    !    Input/output, integer(i4) SEED, the "seed" value, which should
    ...
    !
    r8_uniform_01 = real ( seed, kind=dp ) * 4.656612875E-10
  ...
  subroutine r83_mxv ( n, a, x, b )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the order of the linear system.
    !
    !    Input, real(dp) A(3,N), the R83 matrix.
    !
    !    Input, real(dp) X(N), the vector to be multiplied by A.
    !
    !    Output, real(dp) B(N), the product A * x.
    !
    implicit none
  ...
  subroutine r83_uniform ( n, seed, a )
    ...
    !    Input, integer(i4) N, the order of the linear system.
    !
    !    Input/output, integer(i4) SEED, a seed for the random number
    !    generator.
    !
    !    Output, real(dp) A(3,N), the R83 matrix.
    !
    implicit none
  ...
  subroutine r85_print ( n, a, title )
    ...
    !    N must be positive.
    !
    !    Input, real(dp) A(5,N), the matrix.
    !
  ...
  subroutine r85_print_some ( n, a, ilo, jlo, ihi, jhi, title )
    ...
    !    Input, real(dp) A(5,N), the matrix.
    !
    !    Input, integer(i4) ILO, JLO, IHI, JHI, the first row and
  ...
  subroutine r8ge_fs ( n, a, b, info )
    ...
    !
    !    Output, integer(i4) INFO, singularity flag.
...
!    integer(i4) j
  ...
  subroutine r8vec_bracket ( n, x, xval, left, right )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, length of input array.
    !
    !    Input, real(dp) X(N), an array sorted into ascending order.
    !
    !    Input, real(dp) XVAL, a value to be bracketed.
    !
    !    Output, integer(i4) LEFT, RIGHT, the results of the search.
    !    Either:
    !      XVAL < X(1), when LEFT = 1, RIGHT = 2;
    !      X(N) < XVAL, when LEFT = N-1, RIGHT = N;
    !    or
  ...
  subroutine r8vec_bracket3 ( n, t, tval, left )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, length of the input array.
    !
    !    Input, real(dp) T(N), an array sorted into ascending order.
    !
    !    Input, real(dp) TVAL, a value to be bracketed by entries of T.
    !
    !    Input/output, integer(i4) LEFT.
    !
  ...
  function r8vec_distinct ( n, x )
    ...
    !
    !    Input, real(dp) X(N), the vector to be checked.
    !
  ...
  subroutine r8vec_even ( n, alo, ahi, a )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of values.
    !
    !    Input, real(dp) ALO, AHI, the low and high values.
    !
    !    Output, real(dp) A(N), N evenly spaced values.
  ...
  subroutine r8vec_indicator ( n, a )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of elements of A.
    !
    !    Output, real(dp) A(N), the array to be initialized.
    !
    implicit none
  ...
  subroutine r8vec_order_type ( n, a, order )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of entries of the array.
    !
    !    Input, real(dp) A(N), the array to be checked.
    !
    !    Output, integer(i4) ORDER, order indicator:
  ...
  subroutine r8vec_sort_bubble_a ( n, a )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of entries in the array.
    !
    !    Input/output, real(dp) A(N).
  ...
  subroutine r8vec_uniform_01 ( n, seed, r )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) M, the number of entries in the vector.
    !
    !    Input/output, integer(i4) SEED, the "seed" value, which should
    ...
    do i = 1, n
       ...
       r(i) = real ( seed, kind=dp ) * 4.656612875D-10
  ...
  subroutine r8vec_unique_count ( n, a, tol, unique_num )
    ...
    !    Input, integer(i4) N, the number of elements of A.
    !
    !    Input, real(dp) A(N), the unsorted array to examine.
    !
    !    Input, real(dp) TOL, a nonnegative tolerance for equality.
    !    Set it to 0.0 for the strictest test.
    !
    !    Output, integer(i4) UNIQUE_NUM, the number of unique elements.
    !
    implicit none
  ...
  subroutine spline_b_val ( ndata, tdata, ydata, tval, yval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) NDATA, the number of data values.
    !
    !    Input, real(dp) TDATA(NDATA), the abscissas of the data.
    !
    !    Input, real(dp) YDATA(NDATA), the data values.
    !
    !    Input, real(dp) TVAL, a point at which the spline is
    !    to be evaluated.
    !
    !    Output, real(dp) YVAL, the value of the function at TVAL.
    !
  ...
  subroutine spline_bezier_val ( dim_num, interval_num, data_val, point_num, &
    ...
    !  Parameters:
    !
    !    Input, integer(i4) DIM_NUM, the spatial dimension.
    !
    !    Input, integer(i4) INTERVAL_NUM, the number of intervals.
    !
    !    Input, real(dp) DATA_VAL(DIM_NUM,3*INTERVAL_NUM+1), the control
    !    values.
    !
    !    Input, integer(i4) POINT_NUM, the number of sample points at
    !    which the Bezier cubic spline is to be evaluated.
    !
    !    Input, real(dp) POINT_T(POINT_NUM), the "T" values associated
    ...
    !
    !    Output, real(dp) POINT_VAL(DIM_NUM,POINT_NUM), the value
    !    of the Bezier cubic spline at the sample points.
    !
    implicit none

    integer(i4), parameter :: cubic = 3
    integer(i4) interval_num
    integer(i4) dim_num
    integer(i4) point_num
    ...
    do point = 1, point_num
       ...
       t_01 = t - real ( interval - 1, kind=dp )
  ...
  subroutine spline_constant_val ( ndata, tdata, ydata, tval, yval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) NDATA, the number of data points defining
    !    the spline.  NDATA must be at least 1.
    !
    !    Input, real(dp) TDATA(NDATA-1), the breakpoints.  The values
    !    of TDATA should be distinct and increasing.
    !
    !    Input, real(dp) YDATA(NDATA), the values of the spline in
    !    the intervals defined by the breakpoints.
    !
    !    Input, real(dp) TVAL, the point at which the spline is
    !    to be evaluated.
    !
    !    Output, real(dp) YVAL, the value of the spline at TVAL.
    !
  ...
  subroutine spline_cubic_set_full ( n, t, y, ibcbeg, ybcbeg, ibcend, ybcend, ypp )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of data points; N must be
    !    at least 2.
    !
    !    Input, real(dp) T(N), the points where data is specified.
    !    The values should be distinct, and increasing.
    !
    !    Input, real(dp) Y(N), the data values to be interpolated.
    !
    !    Input, integer(i4) IBCBEG, the left boundary condition flag:
    ...
    !
    !    Input, real(dp) YBCBEG, the left boundary value, if needed.
    !
    !    Input, integer(i4) IBCEND, the right boundary condition flag:
    ...
    !
    !    Input, real(dp) YBCEND, the right boundary value, if needed.
    !
    !    Output, real(dp) YPP(N), the second derivatives of
    !    the cubic spline.
    !
    implicit none
    ...
    do i = 2, n - 1
       b(i) = ( y(i+1) - y(i) ) / ( t(i+1) - t(i) ) &
            - ( y(i) - y(i-1) ) / ( t(i) - t(i-1) )
       a(i,i-1) = ( t(i)   - t(i-1) ) / 6.0_dp
       a(i,i)   = ( t(i+1) - t(i-1) ) / 3.0_dp
       a(i,i+1) = ( t(i+1) - t(i)   ) / 6.0_dp
    end do
    !
    ...
    if ( ibcend == 0 ) then
       ...
       a(n,n-1) = ( t(n) - t(n-1) ) / 6.0_dp
       a(n,n) = ( t(n) - t(n-1) ) / 3.0_dp
...
!    integer(i4) info
  ...
  subroutine spline_cubic_val ( n, t, y, ypp, tval, yval, ypval, yppval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of data values.
    !
  ...
  subroutine spline_cubic_val2 ( n, t, y, ypp, left, tval, yval, ypval, yppval )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of knots.
    !
    !    Input, real(dp) T(N), the knot values.
    !
    !    Input, real(dp) Y(N), the data values at the knots.
    !
    !    Input, real(dp) YPP(N), the second derivatives of the spline at
    !    the knots.
    !
    !    Input/output, integer(i4) LEFT, the suggested T interval to 
    ...
    !
    !    Input, real(dp) TVAL, a point, typically between T(1) and T(N), at
  ...
  subroutine spline_hermite_set ( ndata, tdata, ydata, ypdata, c )
    ...
    !
    !    Input, real(dp) TDATA(NDATA), the abscissas of the data points.
    !    The entries of TDATA are assumed to be strictly increasing.
    !
    !    Input, real(dp) Y(NDATA), YP(NDATA), the value of the
    !    function and its derivative at TDATA(1:NDATA).
    !
    !    Output, real(dp) C(4,NDATA), the coefficients of the
    !    Hermite polynomial.
  ...
  subroutine spline_hermite_val ( ndata, tdata, c, tval, sval, spval )
    ...
    !    The entries of TDATA are assumed to be strictly increasing.
    !
    !    Input, real(dp) C(4,NDATA), the coefficient data computed by
    !    SPLINE_HERMITE_SET.
    !
    !    Input, real(dp) TVAL, the point where the interpolant is to
    !    be evaluated.
    !
    !    Output, real(dp) SVAL, SPVAL, the value of the interpolant
  ...
  subroutine spline_linear_int ( ndata, tdata, ydata, a, b, int_val )
    ...
    !
    !    Input, real(dp) TDATA(NDATA), YDATA(NDATA), the values of
    !    the independent and dependent variables at the data points.  The
    !    values of TDATA should be distinct and increasing.
    !
    !    Input, real(dp) A, B, the interval over which the
    !    integral is desired.
    !
    !    Output, real(dp) INT_VAL, the value of the integral.
    !
    implicit none
  ...
  subroutine spline_linear_intset ( n, int_x, int_v, data_x, data_y )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the number of intervals.
    !
    !    Input, real(dp) INT_X(N+1), the points that define the intervals.
    !    Interval I lies between INT_X(I) and INT_X(I+1).
    !
    !    Input, real(dp) INT_V(N), the desired value of the integral of the
    !    linear spline over each interval.
    !
    !    Output, real(dp) DATA_X(N), DATA_Y(N), the values of the
  ...
  subroutine spline_linear_val ( ndata, tdata, ydata, tval, yval, ypval )
    ...
    !    to be evaluated.
    !
    !    Output, real(dp) YVAL, YPVAL, the value of the spline and
  ...
  subroutine spline_overhauser_nonuni_val ( ndata, tdata, ydata, tval, yval )
    ...
    !    Input, real(dp) YDATA(NDATA), the data values.
    !
    !    Input, real(dp) TVAL, the value where the spline is to
    !    be evaluated.
    !
    !    Output, real(dp) YVAL, the value of the spline at TVAL.
    !
  ...
  subroutine spline_overhauser_val ( dim_num, ndata, tdata, ydata, tval, yval )
    ...
    !    corresponding to the abscissas.
    !
    !    Input, real(dp) TVAL, the abscissa value at which the spline
    !    is to be evaluated.  Normally, TDATA(1) <= TVAL <= T(NDATA), and
    !    the data will be interpolated.  For TVAL outside this range,
    !    extrapolation will be used.
    !
    !    Output, real(dp) YVAL(DIM_NUM), the value of the spline at TVAL.
    !
    implicit none
  ...
  subroutine spline_pchip_set ( n, x, f, d )
    ...
    !    variable values.
    !
    !    Input, real(dp) F(N), dependent variable values to be
    ...
    !    direction.
    !
    !    Output, real(dp) D(N), the derivative values at the
  ...
  subroutine spline_pchip_val ( n, x, f, d, ne, xe, fe )
    ...
    !    variable values.
    !
    !    Input, real(dp) F(N), the function values.
    !
    !    Input, real(dp) D(N), the derivative values.
    !
    !    Input, integer(i4) NE, the number of evaluation points.
    !
    !    Input, real(dp) XE(NE), points at which the function is to
    !    be evaluated.
    !
  ...
  subroutine spline_quadratic_val ( ndata, tdata, ydata, tval, yval, ypval )
    ...
    !    values of TDATA should be distinct and increasing.
    !
    !    Input, real(dp) TVAL, the point at which the spline is to
    !    be evaluated.
    !
    !    Output, real(dp) YVAL, YPVAL, the value of the spline and
    !    its first derivative dYdT at TVAL.  YPVAL is not reliable if TVAL
