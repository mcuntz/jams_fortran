...
MODULE mo_sobol
  ...
  PUBLIC :: sobol       ! Quasirandom Sobol vector.
  PUBLIC :: sobol_array ! Multiple, subsequent quasirandom Sobol vectors.
  ...
  !     INTENT(INOUT)
  !>        \param[in,out] "integer(i4/i8) :: seed"       the "seed" for the sequence. \n
  ...
  !     INTENT(OUT)
  !>        \param[out] "real(sp/dp) :: quasi(dim_num)"       the next quasirandom vector
  !
  !     INTENT(IN), OPTIONAL
  !         None
  !
  ...
  !
  !     RESTRICTIONS
  !>       \note This code can not be used in parallel mode.
  !
  !     EXAMPLE
  !         dims_i4 = 3
  !         seed_i4 = 0
  !         call sobol( dims_i4, seed_i4, sobol_set_sp ) ---> sobol_set_sp = 0.0000000   0.0000000   0.0000000
  !         call sobol( dims_i4, seed_i4, sobol_set_sp ) ---> sobol_set_sp = 0.5000000   0.5000000   0.5000000
  !         call sobol( dims_i4, seed_i4, sobol_set_sp ) ---> sobol_set_sp = 0.7500000   0.2500000   0.7500000
  ...
  !     PURPOSE
  !
  !>        \brief Multiple, subsequent quasirandom Sobol vectors.
  !
  !>        \details Generates a sequence of quasirandom Sobol vectors. \n
  ...
  !>                                                      m must satisfy 1 <= m <= 1111
  !>        \param[in] "integer(i4) :: n"                 Number of points to generate.
  !>        \param[in] "integer(i4) :: skip"              Number of initial points to skip.

  !     INTENT(INOUT)
  !         None

  !     INTENT(OUT)
  !>        \param[out] "real(sp/dp) :: r(n,m)"           Sobol dataset
...
CONTAINS
  ...
  subroutine i4_sobol ( dim_num, seed, quasi )
    ...
    integer(i4),                     intent(in)       :: dim_num
    integer(i4),                     intent(inout)    :: seed
    real(sp),    dimension(dim_num), intent(out)      :: quasi

    ! local
    integer(i4), parameter                            :: dim_max = 1111
    integer(i4), parameter                            :: log_max = 30
    integer(i4)                                       :: atmost
    integer(i4), save                                 :: dim_num_save = -1
    integer(i4)                                       :: i
    ...
    integer(i4)                                       :: k
    integer(i4)                                       :: l
    integer(i4), save, dimension(dim_max)             :: lastq
    integer(i4)                                       :: m
    integer(i4), save                                 :: maxcol
    integer(i4)                                       :: newv
    integer(i4), save, dimension(1:dim_max)           :: poly
    real(sp),    save                                 :: recipd
    integer(i4), save                                 :: seed_save = - 1
    integer(i4)                                       :: seed_temp
    integer(i4), save, dimension(1:dim_max,1:log_max) :: v

    !$ write(*,*) 'mo_sobol: subroutine sobol can not be used in parallel'
    !$ stop
    ...
    !
    quasi(1:dim_num) = real ( lastq(1:dim_num), sp ) * recipd
    lastq(1:dim_num) = ieor ( lastq(1:dim_num), v(1:dim_num,l) )
  ...
  subroutine i8_sobol ( dim_num, seed, quasi )
    ...
    integer(i8),                     intent(in)       :: dim_num
    integer(i8),                     intent(inout)    :: seed
    real(dp),    dimension(dim_num), intent(out)      :: quasi

    ! local
    integer(i8), parameter                            :: dim_max = 1111
    integer(i8), parameter                            :: log_max = 62
    integer(i8)                                       :: atmost
    integer(i8), save                                 :: dim_num_save = 0
    integer(i8)                                       :: i
    logical,           dimension(log_max)             :: includ
    logical,     save                                 :: initialized = .false.
    integer(i8)                                       :: j
    integer(i8)                                       :: j2
    integer(i8)                                       :: k
    integer(i8)                                       :: l
    integer(i8), save, dimension(dim_max)             :: lastq
    integer(i8)                                       :: m
    integer(i8), save                                 :: maxcol
    integer(i8)                                       :: newv
    integer(i8), save, dimension(1:dim_max)           :: poly
    real(dp),    save                                 :: recipd
    integer(i8), save                                 :: seed_save = - 1
    integer(i8)                                       :: seed_temp
    integer(i8), save, dimension(1:dim_max,1:log_max) :: v
    ...
    if ( seed == 0_i8 ) then
       ...
       l = 1_i8
       lastq(1:dim_num) = 0_i8

    else if ( seed == seed_save + 1_i8 ) then
       !
       !  Find the position of the right-hand zero in SEED.
       !
       l = i8_bit_lo0( seed )
       ...
       do seed_temp = seed_save, seed - 1_i8
       ...
       do seed_temp = seed_save+1, seed - 1_i8
          l = i8_bit_lo0( seed_temp )
          lastq(1:dim_num) = ieor ( lastq(1:dim_num), v(1:dim_num,l) )
       end do
    ...
    !  Calculate the new components of QUASI.
    !
    quasi(1:dim_num) = real ( lastq(1:dim_num), dp ) * recipd
    lastq(1:dim_num) = ieor ( lastq(1:dim_num), v(1:dim_num,l) )
  ...
  function i4_bit_hi1( n )
    ...
    !  Parameters:
    !
    !    Input, integer(i4) N, the integer to be measured.
    !    N should be nonnegative.  If N is nonpositive, I4_BIT_HI1
    !    will always be 0.
    !
    !    Output, integer(i4) I4_BIT_HI1, the number of bits base 2.
    !
    implicit none

    integer(i4) :: bit
    integer(i4) :: i4_bit_hi1
    integer(i4) :: i
    integer(i4) :: n
  ...
  function i4_bit_lo0( n )
    ...
    !
    !    Output, integer(i4) I4_BIT_LO0, the position of the low 1 bit.
    !
    implicit none
  ...
  function i8_bit_hi1( n )
    ...
    !    be variables of type integer(i8)!
    !
    !  Example:
    !
    !       N    Binary    Hi 1
    ...
    !  Parameters:
    !
    !    Input, integer(i8) N, the integer to be measured.
    !    N should be nonnegative.  If N is nonpositive, I8_BIT_HI1
    !    will always be 0.
    !
    !    Output, integer(i8) I8_BIT_HI1, the number of bits base 2.
    !
    implicit none

    integer(i8) :: bit
    integer(i8) :: i8_bit_hi1
    integer(i8) :: i
    integer(i8) :: n
  ...
  function i8_bit_lo0( n )
    ...
    !    N should be nonnegative.
    !
    !    Output, integer(i8) I8_BIT_LO0, the position of the low 1 bit.
    !
    implicit none
  ...
  !   !  Parameters:
  !   !
  !   !    Input, integer(i4) DIM_NUM, the spatial dimension.  Only values
  !   !    of 1 through 13 will result in useful responses.
  !   !
  !   !    Output, integer(i4) TAU_SOBOL, the value TAU.
  !   !
  !   implicit none

  !   integer(i4), parameter                   :: dim_max = 13

  !   integer(i4)                              :: dim_num
  !   integer(i4), save, dimension ( dim_max ) :: tau_table = (/ &
  !        0,  0,  1,  3,  5, &
  !        8, 11, 15, 19, 23, &
  !        27, 31, 35 /)
  !   integer(i4)                              :: tau_sobol
